...
export class Vector2 {
	...
	export class Turtle {
		...
		runStepByStep(stepsPerSecond: number = 1, callback: Function | void) {
			if (stepsPerSecond <= 0) {
		...
		private nextStep(stepsPerSecond: number, iteration: number, callback: Function | void) {
		...
		savePos() {
			if (!this.executing) {
				this.actions.push({ action: this.savePos.bind(this), param: new Vector2(this.x, this.y) });
				return;
			}
		...
		restorePos() {
			if (!this.executing) {
				this.actions.push({ action: this.restorePos.bind(this), param: new Vector2(this.x, this.y) });
				return;
			}
